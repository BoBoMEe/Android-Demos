...
public class NetworkStatusManager {
  ...
  /**
   * If the most recent connectivity event was a DISCONNECT, return any information supplied in the
   * broadcast about an alternate network that might be available. If this returns a non-null
   * value,
   * then another broadcast should follow shortly indicating whether connection to the other
   * network
   * succeeded.
   *
   * @return NetworkInfo
  ...
  public enum State {
    ...
    CONNECTED,
    /**
     * This state is returned if there is no connectivity to any network. This is set to true under
     * two circumstances:
     * <ul>
     * <li>When connectivity is lost to one network, and there is no other available network to
     * attempt to switch to.</li>
     * <li>When connectivity is lost to one network, and the attempt to switch to another network
     * fails.</li>
     */
    NOT_CONNECTED
  }
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if (BuildConfig.DEBUG) {
        Log.d(TAG, "onReceive(): mNetworkInfo="
            ...
            : mOtherNetworkInfo + " noConn=" + noConnectivity)
            + " mState="
            + mState.toString());
      }
   ...
   * to network setting
   */
  public static void toNetWorkSettings(Context paramContext) {
